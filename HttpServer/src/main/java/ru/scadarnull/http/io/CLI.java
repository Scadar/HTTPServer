...
import ru.scadarnull.http.io.impl.HttpServerFactory;
...
public class CLI {
    ...
    private static final List<String> QUIT_CMD = Collections.unmodifiableList(Arrays.asList("q", "quit", "exit"));
    ...
    private static void waitForStopCommand(HttpServer httpServer) {
        try(Scanner scanner = new Scanner(System.in, StandardCharsets.UTF_8.name())){
            while(true){
                ...
                if(QUIT_CMD.contains(cmd.toLowerCase())){
                    httpServer.stop();
                    break;
                }else{
                    LOGGER.error("Unsupported command: " + cmd + ". To shutdown server please type: q");
                }
            }
        }
    }
